...
| **Middleware / Proxy** | Code that runs before a page loads, to check things like "is this person logged in?" | An edge function that intercepts requests before they reach a route handler or page. In this Next.js version the file is `proxy.ts` instead of the classic `middleware.ts`. |
